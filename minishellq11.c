...
int main(){
    ...
    pid_t pidFils;
    int pipes_nb; /*nb de pipes nécessaires lors de pipelines*/
    int tube[NB_TUBE_MAX][2];
        ...
        if(commande != NULL) {
            ...
            //printf("La commande est vide\n");
                ...
                //On regarde si la commande est susp pour suspendre le minishell
                    ...
                    pipes_nb = 0;
                    //Création des tubes entre les différents processus fils
                    while(commande->seq[pipes_nb + 1] != NULL) {
                        pipe(tube[pipes_nb]);
                        pipes_nb++;
                    }

                    // Creation des processus fils du même père en fonction des commandes(proc) entrées.
                    ...
                    for (int i = 0; i <= pipes_nb; i++) {
                        ...
                        if (pidFils < 0) {
                            ...
                            //On ferme les tubes qu'on utilisera pas
                            for (int j = 0; j < pipes_nb; j++) {
                                if(j == i) { //cas processus à la fin de la ligne de commande qui devra rediriger tube vers sortie standard
                                    close(tube[j][0]);
                                    dup2(tube[j][1], 1);
                                    close(tube[j][1]);
                                } else if(j == i - 1) { // cas processus au debut de la cmd qui devra rediriger entree standard vers tube
                                    close(tube[j][1]);
                                    dup2(tube[j][0], 0);
                                    close(tube[j][0]);
                                } else { 
                            ...
                            //redirection >
                            if (i == pipes_nb && commande->out != NULL) {
                                //on redirige la dernière écriture vers la sortie standard
                            ...
                            //Fermeture de tous les tubes, non utilisés par le pere
                            for (int j = 0; j < pipes_nb; j++){
                                close(tube[j][0]);
                                close(tube[j][1]);
                            }
